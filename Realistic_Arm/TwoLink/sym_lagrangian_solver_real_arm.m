...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sym_lagrangian_solver_real_arm(T,V,fric,x,u)
...
% Calculate matrix C through Christoffel Symbols of the first kind
...
matlabFunction(phi,     'vars',{x,u},'file','phi_fun');
...
phi_jac_x = jacobian(phi,x);
matlabFunction(phi_jac_x,'vars',{x,u},'file','phi_jac_x_fun');

% Jacobian of phi w.r.t. u
phi_jac_u = jacobian(phi,u);
matlabFunction(phi_jac_u,'vars',{x,u},'file','phi_jac_u_fun');

% Jacobian of friction w.r.t x
fric_jac = jacobian(fric,x);
matlabFunction(fric_jac,'vars',{x},'file','fric_jac_fun');

%get the k2 and k4 for the inversekimetic 
K2=solve(phi(1)==0,u(2));
K4=solve(phi(2)==0,u(4));
matlabFunction(K2,'vars',{x,u},'file','K2_fun');%matlabfunction of the k2 w.r.t k1
matlabFunction(K4,'vars',{x,u},'file','K4_fun');%matlabfunction of the k4 w.r.t k3
